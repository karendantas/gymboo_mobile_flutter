...
final homeDataProvider = FutureProvider<HomeData>((ref) async {
  ...
  final results = await (
    ref.watch(petRepositoryProvider).getMyPet(user.id),
    ref.watch(goalRepositoryProvider).getMyGoal(user.id),
    ref.watch(goalRepositoryProvider).getWeeklyProgress(user.id),
  ).wait;
